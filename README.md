...
面向单片机的锂电池 **SOC（荷电状态）+ SOH（健康度）** 在线估计算法库（纯 C99），采用EKF+触发式学习一阶RC电池模型进行估计，
配套一套用于自动采集、拟合、标定、参数辨识与离线算法验证的 PC 工具链（Python）和上位机。

参考硬件：18650（NCR18650GA / 18650-3350D，标称 3350 mAh）
+ 采集板（INA226 + NTC）+ 一台程控电子负载（RS232 / SCPI）。
